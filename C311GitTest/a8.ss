...
(define rember
  (lambda (x ls)
    (call/cc (lambda (jumpout)
               (tramp (rembert x ls (empty-kt jumpout)))))))
...
(define multi-rembero
  (lambda (x ls k)
    (cond
      ((null? ls) (apply-k1 k '()))
      ((eq? (car ls) x) (multi-rembero x (cdr ls) k))
      (else (multi-rembero x (cdr ls)
                (other-k1 (car ls) k))))))

(define multi-rember
  (lambda(x ls)
    (call/cc (lambda (jumpout)
               (tramp (multi-rembert x ls (empty-kt jumpout)))))))

(define multi-rembert
  (lambda (x ls k)
    (lambda ()
      (cond
        ((null? ls) (apply-k1 k '()))
        ((eq? (car ls) x) (multi-rembert x (cdr ls) k))
        (else (multi-rembert x (cdr ls)
                (other-k1 (car ls) k)))))))
...
(trace-define Mt
  (lambda (f k)
    (apply-k7 k (lambda (ls k)
                  (cond
                    [(null? ls) (apply-k7 k '())]
                    [else (Mt f (outer-k7 k ls))])))))
